...
/*
 * Basic runtime and utility functions 基本的运行和效用函数
 */
...
/*
	...
	struct of this type.
	给一个结构体分配空间，清除空间，标记这个指针使其属于这种类型
	...
	返回分配空间的结构体指针
*/
/* alloc and zero a struct, and tag it for memento 分配并清零一个结构 标记这个指针作为记忆碎片*/
...
typedef struct fz_storable_s fz_storable;//一个Resource store，存放解码的对象，起到catch缓存的坐用
...
{
	if (stm->rp == stm->wp)//stm是一段缓存，rp与wp之间的数据是有效的
	{
		fz_fill_buffer(stm);
...
/*
	...
	如果使用线性插值绘制图像，则布尔标记设置为非零，如果图像使用最近邻抽样，则设置为零。
 ...
 * a,b,c,d,e,f一起构成字形变换矩阵

 * Glyphs are referenced by glyph ID.
 * The Unicode text equivalent is kept in a separate array
 * with indexes into the glyph array.
   字形是由字形ID引用的，Unicode文本等效保存在一个单独的数组中，索引包含在字形数组中。
 */
...
struct fz_shade_s
{
	...
	int use_function;
	float function[256][FZ_MAX_COLORS + 1];//256*9的数组.最大颜色
