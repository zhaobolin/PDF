...
struct zblroute//图形状态栈中的一条路径，可能与其他路径共用剪辑路径或者转置矩阵等
{
	...
	float color[4];//记录颜色
	int scolorspace;
	float *scolor;//特殊情况,既轮廓上色又填充颜色

	int drawingmethord;//绘制方法 1-S 2-f* 3-f/F 4-s 5-B 6-B* 7-b 8-b*
	
	int countpoint;//记录路径点个数
	struct routepoint* pointheadler;//p0 p1分别存放直线路径点的x,y坐标,贝塞尔则是p0-p6三组点,state构造路径的m,l,h,c状态(分别对应0，1，2，3）
	struct routepoint* currentpoint;//链表的当前指针

	struct zblroute* nextroute;//下个路径
};

struct zblstack//一个图形状态栈
{
	int existstack;//是否是图形状态栈
	int countroute;//记录一个图形状态栈中有多少个路径
	int existnest;//标志位 是否存在嵌套栈 复制与提取时层数的标志位

	int countclip;//记录裁剪路径点数	
	int existclip;//判断标志，此组结构是否存在裁剪路径,以及若存在,存在的是哪种 0-不存在	1-W 2-W*
	struct  routepoint* clipheadler;//存放裁剪路径的指针,与points一样的结构	
	struct  routepoint* currentclip;//当前裁剪路径点指针	
	...
	struct zblroute* routeheadler;//路径头
	struct zblroute* currentroute;//当前路径

	struct zblstack* stackheadler;//栈内嵌套栈链表头  此两项初始置NULL
	struct zblstack* currentstack;//当前嵌套栈

	struct zblstack* nextstack;//下个栈
};
...
struct zblrouteset//提取路径集合结构体 一页中的所有图形信息
{
	int count;
...
extern
void addclippoints(struct zblroute* route)//为裁剪路径添加空点集
{
	...
	route->currentclippoint=route->currentclippoint->nextpoint;//currentclippoint指向新点
}
void addpoints(struct zblroute* route)//为路径添加一个空点 并为current指针赋值
{
	struct routepoint* points=(struct routepoint*)malloc(sizeof(struct routepoint));
	points->nextpoint=NULL;
	points->p0=-1;
	...
	points->p5=-1;
	points->state=-1;
	route->currentpoint->nextpoint=points;//添加点
	route->currentpoint=route->currentpoint->nextpoint;//current指向新点
}
...
void addroute(struct zblrouteset* getline)//为routeset添加一个route
{
	...
	route->points=(struct routepoint*)malloc(sizeof(struct routepoint));//点集链表的头结点
	route->currentpoint=route->points;
	route->currentpoint->nextpoint=NULL;

	route->countclip=0;//裁剪路径计数为空
	route->existclip=0;//默认不存在裁剪路径
	route->clipregion=NULL;//初始化裁剪路径为空
	route->currentclippoint=NULL;//初始化当前裁剪路径点指针为空
...
{
	void *user;
	void *(*malloc)(void *, unsigned int);//void* 类型可以强制转换为任何其它类型的指针
...
struct fz_error_context_s
{
	...
	struct {
		int code;
		jmp_buf buffer;
	/* SumatraPDF: stack overflows happen in pdf_load_page_tree_node and pdf_read_xref_sections 栈溢出在pdf_load_page_tree_node 和 pdf_read_xref_sections这两个地方*/
	} stack[1024];
	char message[256];
};
...
/* SumatraPDF: add filename and line number to errors and warnings 将文件名和行号添加到错误和警告中 */
...
/*
	fz_flush_warnings: Flush any repeated warnings.冲洗任何重复警告
	...
	重复的警告被缓存、计算并随着重复次数的增加最终打印出来。调用fz_flush_warnings来强制打印最近缓存的警告与重复数量，确保在退出应用程序之前打印所有警告。
...
/*
	...
	指定资源存储区的最大容量，作为fz_new_context的参考
...
/*
	...
	全局状态包含堆栈异常，资源存储等内容，大多数MUPDF中的函数使用上下文参数以引用全局状态
	...
	通过一组函数指针提供一个自定义的内存分配器。为标准库分配器设置为null。
	上下文将保留分配器指针，因此它指向的数据不能在上下文的生命周期内被修改或释放。
	分配内存，保有指针，则指针保存的数据将不会丢失
...
*/
fz_context *fz_clone_context(fz_context *ctx);//复制上下文内容，应对多线程
...
/*
	...
	mupdf是刻意保持无任何知识的特定线程的系统。为了安全的多线程操作，依靠回调客户端提供的功能
...
/*
	fz_malloc_array: Allocate a block of (non zeroed未被清零的) memory (with
...
/*
	Safe string functions 安全的字符串函数
*/
/*
	...
	c字符串指针的指针，在退出时更新，以在找到分隔符之后指向字符串的第一个字符。字符串所指向的stringp将由这个叫损坏（如发现分隔符将被0覆盖）
...
/*
	fz_point is a point in a two-dimensional space .fz_point是二维空间中的一个点
*/
typedef struct fz_point_s fz_point;
struct fz_point_s//二维空间中的一个点
{
	float x, y;
};
...
/*
	...
	使用fz_is_empty_rect 和 fz_is_infinite_rect来检查是否是空的或者无限的矩形
	x0, y0: The top left corner.

	x1, y1: The botton right corner.
*/
typedef struct fz_rect_s fz_rect;
struct fz_rect_s//左上角与右下角定义的矩形
{
	float x0, y0;//左上角
	float x1, y1;//右下角
};
...
*/
extern const fz_matrix fz_identity;//使用的转换矩阵
...
/*
	...
	fz_buffer是一个动态分配比特数组的包装
	Buffers have a capacity (the number of bytes storage immediately
	available) and a current size.
	由这个结构可知当前可用比特数以及当前buffer大小
*/
...
/*
	...
	directions.
	fz_stream是一个读取缓冲，可以从两个方向读取

	Streams are reference counted, so references must be dropped
	by a call to fz_close.
	流是会被记录引用次数的，所以引用必须调用fz_close来消去
...
/*
	...
	first, akin to pbms.
	bitmap每个组件有1bit大小，仅用于创建连续映像缓冲器的半色调版本，样品存储MSB第一，类似于pbms
*/
...
/*
	...
	一个抽象的颜色空间，应当被现阶段库的调用者看做是一个黑箱，封装了如何将颜色转换为颜色空间的知识
*/
...
/*
	...
	allocate.
*/
fz_pixmap *fz_new_pixmap(fz_context *ctx, fz_colorspace *cs, int w, int h);//初始化一个pixmap
...
/*
	...
	半色调是一组阈值瓦片，每个component都有一个，每个阈值瓦片是一个pixmap
*/
...
/*
	...
	不同的格式处理程序（pdf，xps等）将页面解释为设备。 这些设备然后可以以各种方式处理它们接收的呼叫流：
	跟踪设备输出调用的调试信息。
	绘图设备将呈现它们。
	列表设备将它们存储在列表中以供稍后回放。
	文本设备执行文本提取和搜索。
...
/*
	Text extraction device: Used for searching, format conversion etc.文本提取设备：用于搜索、格式转换等 
 */
