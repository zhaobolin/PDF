...
/* If AA_BITS is defined, then we assume constant N bits of antialiasing(抗锯齿). We
 * will attempt to provide at least that number of bits of accuracy（精确） in the
...
参见Mike Ababase--图形编程黑皮书（特别是第40章）
 */
...
fz_insert_gel(fz_gel *gel, float fx0, float fy0, float fx1, float fy1)
{
	...
	fx0 = floorf(fx0 * fz_aa_hscale);//取整
