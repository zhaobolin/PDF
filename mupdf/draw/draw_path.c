...
line(fz_gel *gel, fz_matrix *ctm, float x0, float y0, float x1, float y1)
{
	...
	float ty1 = ctm->b * x1 + ctm->d * y1 + ctm->f;//变换矩阵运算
	...
	float xd, float yd, int depth)
{
	...
	bezier(gel, ctm, flatness, xa, ya, xab, yab, xabc, yabc, xabcd, yabcd, depth + 1);//递归调用
	bezier(gel, ctm, flatness, xabcd, yabcd, xbcd, ybcd, xcd, ycd, xd, yd, depth + 1);
}

void
fz_flatten_fill_path(fz_gel *gel, fz_path *path, fz_matrix ctm, float flatness)//填充路径
{
...
//			printf("贝塞尔曲线坐标:%f %f %f %f %f %f\n",x1,y1,x2,y2,x3,y3);
			bezier(gel, &ctm, flatness, cx, cy, x1, y1, x2, y2, x3, y3, 0);//后六位是绘制参数
			cx = x3;
			cy = y3;
			break;
