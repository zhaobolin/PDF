...
/*
 * tokenizer and low-level object parser 分解器和低级别的对象解析 
 */
...
{
	int ofs;	/* file offset 文件偏移量 / objstm object number objectstreams的数量，objectstreams可包含其他Object对象，即嵌套 */
	int gen;	/* generation 代/ objstm index 索引 */
	int stm_ofs;	/* on-disk stream 磁盘上的流*/
	pdf_obj *obj;	/* stored/cached object 存储的对象*/
