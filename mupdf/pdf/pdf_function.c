...
static void
eval_exponential_func(fz_context *ctx, pdf_function *func, float in, float *out)//存储渐变色的内容
//in从0到1,每次增加1/255 out是shade->function[i]
{
	float x = in;
	float tmp;
	int i,j;
	if(zcounter==0)
	{
		for(j=0;j<3;j++)
		{
			currentstackpoint->bcolor[j]=func->u.e.c0[j];
			currentstackpoint->ecolor[j]=func->u.e.c1[j];
		}
	}
	...
	tmp = powf(x, func->u.e.n);//x为1/255的累加 计算x的u.e.n次方

	//printf("\n处理渐变色函数:参数%d\n",func->n);
	for (i = 0; i < func->n; i++)//此处实现了颜色的渐变
	{
		out[i] = func->u.e.c0[i] + tmp * (func->u.e.c1[i] - func->u.e.c0[i]);
		if (func->has_range)
			out[i] = CLAMP(out[i], func->range[i][0], func->range[i][1]);
		//printf("%f %f %f\n",func->u.e.c0[i],func->u.e.c1[i],out[i]);
	}
}
...
//in从0到1,每次增加1/255,第4个参数为1,shade->function[i], shade->colorspace->n
{
	...
	case EXPONENTIAL: eval_exponential_func(ctx, func, *in, out); break;//in从0到1,每次增加1/255 out是shade->function[i]
