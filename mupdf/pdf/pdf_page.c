...
struct info
{
	pdf_obj *resources;//当前page用到的所有资源。如果当前页不用任何资源，则这是个空字典。忽略所有字段则表示继承父节点的资源
	pdf_obj *mediabox;//定义了要显示或打印页面的物理媒介的区域
	pdf_obj *cropbox;//定义了一个可视区域，当前页被显示或打印的时候，它的内容会被这个区域裁剪。默认值就是 MediaBox
	pdf_obj *rotate;//顺时钟旋转的角度数，这个必须是90的整数倍，默认是0
};
...
pdf_load_page_tree_node(pdf_document *xref, pdf_obj *node, struct info info)
{
	...
	fz_try(ctx)
	{
		kids = pdf_dict_gets(node, "Kids");//将孩子节点字典内容加载到pdf_obj中
...
static void
pdf_load_page_tree(pdf_document *xref)//从xref的相关对象中提取页面树的信息
{
	fz_context *ctx = xref->ctx;
	pdf_obj *catalog;//目录
	pdf_obj *pages;//所有页面节点的集合
	pdf_obj *count;//page对象的个数
	struct info info;
...
pdf_page *
pdf_load_page(pdf_document *xref, int number/*页号*/)//加载PDF的一个页与相关资源，在PDF文件中定位页与其资源，在pdf_load_page可以使用pdf_bound_page检索页面的大小，或利用pdf_run_page_ *渲染页面。
{
	fz_context *ctx = xref->ctx;
	pdf_page *page;
	pdf_annot *annot;
	...
	pdf_load_page_tree(xref);//读取页面树的信息
	...
	if (fz_is_empty_rect(mediabox))//页面尺寸无法找到
	{
	...
	/* Snap page->rotate to 0, 90, 180 or 270 快照页>旋转到0, 90, 180 或 270*/
	if (page->rotate < 0)//rotate 顺时针旋转的角度数，为九十度的整数倍
	...
	page->ctm = fz_concat(ctm, fz_translate(-realbox.x0, -realbox.y0));//变换矩阵

	obj = pdf_dict_gets(pageobj, "Annots");//页面注释
	if (obj)
	{
	...
	page->resources = pdf_dict_gets(pageobj, "Resources");//页面使用的资源
	if (page->resources)
		pdf_keep_obj(page->resources);

	obj = pdf_dict_gets(pageobj, "Contents");//Contents 描述页面内容的流
	fz_try(ctx)
	{
		page->contents = pdf_load_page_contents(xref, obj);

		if (pdf_resources_use_blending(ctx, page->resources))//混合资源？
			page->transparency = 1;
